...
async def sign_in_email(
    ...
    print(f"SIGN-IN/EMAIL DEBUG: email='{email}', password exists={bool(password)}")
    print(f"SIGN-IN/EMAIL DEBUG: Full request: {login_request.dict()}")

    # Backend graceful error handling for missing credentials
    if not email or not password:
        print(f"SIGN-IN/EMAIL DEBUG: Validation failed - email empty: '{email}', password empty: '{password}'")
        raise HTTPException(
            status_code=400,
            detail="Missing credentials"
        )
